...
// 導入元件
import SMAv4AssessmentTable from './SMAv4AssessmentTable.vue';
...
function updateTableRowsSample(SMAv4_result) {
  ...
  sample_list.forEach((sample_name) => {
    ...
    const ROW_SMN1 = ROW(
      ...
      SMAv4_result.resultObj.SAMPLE_DATA.smn1[sample_name].ic_peak.peak_rfu,
      SMAv4_result.resultObj.SAMPLE_DATA.smn1[sample_name].tg_peak.peak_rfu,
    ...
    const ROW_SMN2 = ROW(
      ...
      SMAv4_result.resultObj.SAMPLE_DATA.smn2[sample_name].ic_peak.peak_rfu,
      SMAv4_result.resultObj.SAMPLE_DATA.smn2[sample_name].tg_peak.peak_rfu,
  ...
  for (const smn of ['smn1', 'smn2']) {
    for (const std of ['std1', 'std2', 'std3']) {
      ...
      const ROW_STD = ROW(
        ...
        smn_std.group,
        smn_std.smn,
        smn_std.ic_peak.peak_rfu,
        smn_std.tg_peak.peak_rfu,
        smn_std.rfu_diff,
...
function update_standard_data(){
  ...
  standard_data.value = {
    'smn1': {
      ...
      'std3': {'diff': screening_table_rows.value.find(row => row.sample_name === 'std3' && row.smn === 'smn1').diff},
    },
    'smn2': {
      ...
      'std3': {'diff': screening_table_rows.value.find(row => row.sample_name === 'std3' && row.smn === 'smn2').diff},
    },
  }
}
