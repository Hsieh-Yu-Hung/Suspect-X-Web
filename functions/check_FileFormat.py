...
def download_file_from_storage(bucket, file_path):
    ...
    # 本地存放檔案路徑
    local_file_path = os.path.join(LOCAL_DATA_PATH, file_path)
...
class FileFormatChecker:
    ...
    def __init__(self, bucket=None):
        ...
        # 如果 bucket 存在，則設定 bucket, 否則為腳本測試不下載檔案
        if bucket:
          self.bucket = bucket
        else:
          self.bucket = None
    ...
    def check_qsep100_format(self, file_path):
      ...
      if self.bucket:
        # 下載檔案
        downloaded_file_path = download_file_from_storage(self.bucket, file_path)
      else:
      ...
      # 檢查 Excel 檔案
      excel_file_ok = self.checkExcel(downloaded_file_path, target_sheet)
    ...
    def check_tower_format(self, file_path):
      ...
      # 嘗試以 ENCODING_1 讀取檔案
      try:
        with open(downloaded_file_path, 'r', encoding=ENCODING_1) as file:
          lines = file.readlines()
      except Exception as e:
        # 如果讀取失敗，嘗試以 ENCODING_2 讀取檔案
        if isinstance(e, UnicodeDecodeError):
          with open(downloaded_file_path, 'r', encoding=ENCODING_2) as file:
            lines = file.readlines()
        else:
          # 如果讀取失敗，回傳 False
          self.check_status = CheckStatus.not_checked
